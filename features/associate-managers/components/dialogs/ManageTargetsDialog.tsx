...
export function ManageTargetsDialog({
  ...
  const { data: targets, isLoading } = useManagerTargets(managerId);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Set and review monthly targets. The current month&apos;s target drives the
                ...
                <section className="space-y-2">
                  ...
                  {active ? (
                    ...
                    <div className="rounded-lg border border-dashed border-amber-300 bg-amber-50 p-4 text-sm text-amber-800">
                      No target set for the current month. Create one below — it&apos;ll
                      start driving the dashboard immediately.
                    </div>
                  )}
                </section>
    ...
    <div className="rounded-lg border border-[#00695C]/30 bg-[#E0F2F1]/40 p-4">
      ...
      <div className="grid grid-cols-3 gap-3 text-sm">
        <TargetCell
          ...
          value={target.associate_pro_recruited_target}
        />
        <TargetCell
          label="Selling Ass. Pros"
          value={target.selling_associate_pro_target}
        />
        <TargetCell
...
function UpcomingTargetRow({
  ...
  return (
    <div className="flex items-center justify-between gap-3 rounded-lg border border-gray-200 bg-white px-4 py-3">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500">
            {target.associate_pro_recruited_target} recruited ·{" "}
            {target.selling_associate_pro_target} selling ·{" "}
            {target.performance_score_target} score
          </p>
        </div>
      </div>
      <Button variant="ghost" size="sm" onClick={onEdit}>
      ...
      {/* Delete is hidden until BE exposes a deleteManagerTarget mutation. */}
    </div>
  );
}
    ...
    <div className="flex items-center justify-between px-4 py-2.5 text-sm">
      <div className="flex items-center gap-3">
        ...
        <span className="text-gray-500 text-xs tabular-nums">
          {target.associate_pro_recruited_target} /{" "}
          {target.selling_associate_pro_target} /{" "}
          {target.performance_score_target}
        </span>
      </div>
