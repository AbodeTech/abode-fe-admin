...
import { csManagerInitials, csManagerName } from "../../lib/manager-display";
...
/**
 * Super-admin queue of customers with no CS Manager, as a header dialog
 * (APM parity — the queue used to be its own route).
 *
 * Two steps in one dialog rather than a nested "assign" dialog: pick the
 * customers, then pick who takes them. Nested Radix dialogs trap focus
 * awkwardly and the second step needs the first's selection anyway.
 */
export function UnassignedCustomersDialog({ open, onOpenChange }: Props) {
  ...
  const step: "pick-customers" | "pick-manager" =
    pendingIds.length > 0 ? "pick-manager" : "pick-customers";

  // Reset on the way out rather than in an effect on the way in, so the
  // dialog always reopens on step one without a render-phase state write.
  const reset = () => {
    setPendingIds([]);
    setQuery("");
  ...
  const handleAssign = async () => {
    ...
    try {
      await mutateAsync({ customerIds: pendingIds, managerId: pickedManagerId });
  ...
  const filteredManagers = managers.filter((m) => {
    ...
    return (
      csManagerName(m.manager).toLowerCase().includes(q) ||
          ...
          <>
            <DialogHeader>
              ...
              <DialogDescription>
                ...
                bubble to the top — a customer here for more than 7 days is
                overdue.
              </DialogDescription>
            </DialogHeader>
              ...
              <div className="py-1">
                {isLoading ? (
                  ...
                  <UnassignedCustomersTable
                    ...
                    onAssignOne={(c) => setPendingIds([c._id])}
                  />
                )}
              </div>
            </ScrollArea>
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-xs text-gray-500 leading-tight truncate">
                                {m.assignedCustomersCount} customer
                                {m.assignedCustomersCount === 1 ? "" : "s"} ·{" "}
                                {m.assignedPlansCount} active plan
                                {m.assignedPlansCount === 1 ? "" : "s"}
                              </p>
                            </div>
                          </button>
                        </li>
            ...
            <DialogFooter className="sm:justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  disabled={!pickedManagerId || isPending}
                >
                  {isPending && (
