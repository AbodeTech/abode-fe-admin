...
export function PlanDetailDrawer({ plan, open, onOpenChange }: Props) {
  ...
  const onSubmit = (values: OnboardingCallFormValues) => {
    logCall.mutate(
      {
        planId: plan.plan_id,
        outcome: values.outcome,
        land_choice_reason: values.landChoiceReason?.trim() || undefined,
        ...
        <SheetHeader className="border-b border-gray-200 px-6 py-5 pr-12 space-y-3 text-left">
          <div className="flex items-center gap-2.5">
            ...
            <div className="min-w-0">
              <SheetTitle className="text-base text-gray-900 leading-tight flex items-center gap-1.5">
                ...
                {plan.prior_plans_count > 0 && (
                  <span
                    className="inline-flex items-center gap-0.5 rounded-full bg-purple-50 text-purple-700 text-[10px] px-1.5 py-0.5 shrink-0"
                    title={`Repeat buyer · ${plan.prior_plans_count} prior plan${plan.prior_plans_count === 1 ? "" : "s"}`}
                  >
                    <Repeat className="h-2.5 w-2.5" />+{plan.prior_plans_count}
                  </span>
                )}
              </SheetTitle>
          ...
          <div className="rounded-lg border border-gray-200 bg-gray-50 p-3 space-y-2">
            <div className="flex items-baseline justify-between gap-2">
              <p className="text-sm font-medium text-gray-900 leading-tight">{plan.asset}</p>
              <span className="text-[11px] text-gray-500 shrink-0">
                {plan.product === "flex" ? "Flex" : "Full-ownership"}
              </span>
            </div>
            <p className="text-xs text-gray-500">Opened {formatShortDate(plan.purchase_date)}</p>
            <div className="flex flex-wrap items-center gap-1.5 pt-1">
              <PaymentPill status={plan.payment_status} label={plan.payment_label} />
              <OnboardingPill status={plan.onboarding} />
              <AllocationPill status={plan.allocation} label={plan.allocation_label} />
              <DoaPill status={plan.doa} label={plan.doa_label} />
            </div>
          </div>
          ...
          <section className="space-y-3">
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-gray-900">Onboarding calls</h3>
              </div>
              <span
                className={cn(
                  "text-[11px] rounded-full px-2 py-0.5 font-medium",
                  onboarded ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"
                )}
              >
            ...
            {attemptsLoading ? (
              ...
              <ol className="space-y-2">
                {attempts.map((a) => (
                  <li key={a.id} className="rounded-lg border border-gray-200 p-3 space-y-1.5">
                    <div className="flex items-center justify-between gap-2">
                      <span
                        className={cn(
                          "text-[11px] rounded-full px-2 py-0.5 font-medium",
                    ...
                    {a.land_choice_reason && (
                      <div>
                        <p className="text-[10px] uppercase tracking-wide text-gray-400">Why this land</p>
            ...
            <Form {...form}>
              ...
              >
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="space-y-2">
                      <FormLabel className="text-xs text-gray-600">What happened?</FormLabel>
                      <FormControl>
                            ...
                            return (
                              <button
                                ...
                                className={cn(
                                  ...
                                  active ? "border-[#00695C] bg-[#E0F2F1]/50" : "border-gray-200 hover:border-gray-300"
                                )}
                              >
                                <span className="flex items-center gap-1.5">
                                  <Icon className={cn("h-3.5 w-3.5", o.tone)} />
                                  <span className="text-xs font-medium text-gray-900">{o.label}</span>
                                </span>
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-xs text-gray-600">
                        ...
                        {outcome === "done" && <span className="text-[#AD1F2A]"> *</span>}
                      </FormLabel>
                      <FormControl>
                        <Textarea
                          {...field}
                ...
                <div className="flex justify-end">
                  <Button type="submit" size="sm" className="h-8 text-xs" disabled={logCall.isPending}>
                    {logCall.isPending && <Loader2 className="h-3 w-3 animate-spin" />}
                    Log call
                  </Button>
                </div>
              </form>
