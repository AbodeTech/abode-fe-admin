...
import { Plus, Pencil, Target, CalendarDays, Clock, Loader2 } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
type FormState = { mode: "closed" } | { mode: "create" } | { mode: "edit"; target: CSManagerTarget };

const MONTHS = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];

const formatPeriodLabel = (t: CSManagerTarget) => `${MONTHS[t.month - 1]} ${t.year}`;
...
const bucketOf = (t: CSManagerTarget): Bucket => {
  const now = new Date();
  const currentMonth = now.getMonth() + 1;
  const currentYear = now.getFullYear();
  if (t.year === currentYear && t.month === currentMonth) return "active";
  if (t.year > currentYear || (t.year === currentYear && t.month > currentMonth)) return "upcoming";
  return "past";
};

export function ManageCSTargetsDialog({ open, onOpenChange, managerId, managerName }: Props) {
  ...
  const { active, upcoming, past } = useMemo(() => {
    ...
    return {
      ...
      upcoming: all.filter((t) => bucketOf(t) === "upcoming").sort((a, b) => a.year - b.year || a.month - b.month),
      past: all.filter((t) => bucketOf(t) === "past").sort((a, b) => b.year - a.year || b.month - a.month),
    };
  }, [targets]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Set and review monthly targets for allocated, onboarded, and deeds delivered.
          </DialogDescription>
        </DialogHeader>
            ...
            {isLoading ? (
              ...
              <>
                <section className="space-y-2">
                  ...
                  {active ? (
                    <ActiveTargetCard target={active} onEdit={() => setForm({ mode: "edit", target: active })} />
                  ) : (
                ...
                {upcoming.length > 0 && (
                  <section className="space-y-2">
                    ...
                    <div className="space-y-2">
                      {upcoming.map((t) => (
                        <UpcomingTargetRow key={t.id} target={t} onEdit={() => setForm({ mode: "edit", target: t })} />
                      ))}
                    </div>
                  </section>
                )}
                  ...
                  <section className="space-y-2">
                    ...
                    <div className="rounded-lg border border-gray-200 bg-white divide-y divide-gray-100">
                      {(showAllHistory ? past : past.slice(0, 3)).map((t) => (
                        <PastTargetRow key={t.id} target={t} />
                      ))}
                    </div>
                  </section>
                )}
...
function ActiveTargetCard({ target, onEdit }: { target: CSManagerTarget; onEdit: () => void }) {
...
function UpcomingTargetRow({ target, onEdit }: { target: CSManagerTarget; onEdit: () => void }) {
  return (
...
function PastTargetRow({ target }: { target: CSManagerTarget }) {
  return (
